<?php
	...
	sendEmails($mysqli);
	removeResponses($mysqli);
		
	function removeResponses($mysqli){
		$query = 'SELECT * FROM settings WHERE remove_responses = 1';
		...
		foreach($teachers as $teacher){
			...
			if($teacher['category_id']!='0') $query.=(' AND p.category_id = '.$teacher['category_id']);
			if($teacher['keep_pin']=='1') $query.=' AND r.response_pinned != 1';
			$query.=(' AND DATEDIFF(CURDATE(),r.response_date)>'.$teacher['remove_after']);
	...
	function sendEmails($mysqli){
		...
		WHERE send_reports = 1 AND send_reports_time = 0 AND ((send_reports_type = 0) 
